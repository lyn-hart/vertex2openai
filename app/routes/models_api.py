...
async def list_models(fastapi_request: Request, api_key: str = Depends(get_api_key)):
    ...
    def add_model_and_variants(base_id: str, prefix: str = ""):
        ...
        for suffix in suffixes:
            ...
            # Experimental models have no prefix; Express models are listed bare (no [EXPRESS]).
    ...
    # Express models: bare native names (no [EXPRESS] prefix)
    if has_express_key:
        for model_id in raw_express_models:
            add_model_and_variants(model_id, "")

    # SA (PAY) models keep [PAY] prefix so callers can force SA credentials
    if has_sa_creds:
